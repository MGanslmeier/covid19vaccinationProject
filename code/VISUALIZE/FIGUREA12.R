setwd("~/Dropbox/covidVaccine/8sub/1_NATURE/UPDATE/REPOSITORY")
...
files <- list.files('res', full.names = T, recursive = T) %>%
  subset(., grepl('txt', .) & grepl('extended\\/FIGUREA12', .))
...
for(i in 1:length(files)){
  ...
  IVCONTROL <- temp %>% subset(., V1 %in% c('IVCONTROL')) %>% t() %>% as.data.frame() %>% set_names('obs') %>% .[-1,] %>% gsub(',', '', .)
  
  df <- temp[(index_vacc):(index[2]-2), ] %>%
    subset(., V1 != '') %>% t(.) %>% as.data.frame() %>%
    set_names(gsub(' ', '', .[1,])) %>% .[-1,] %>%
    mutate(DEP = DEP, LAG = LAG, FSTAT = FSTAT, GLOBALTERM = GLOBALTERM, 
           LOCALTERM1 = LOCALTERM1, IVCONTROL = IVCONTROL) %>%
    gather(., INDEP, coef, -c(DEP, LAG, GLOBALTERM, LOCALTERM1, IVCONTROL, FSTAT)) %>%
    ...
    mutate(INST = paste(GLOBALTERM, LOCALTERM1, IVCONTROL, sep = ' - ')) %>%
    bind_rows(., df)
}
df <- df %>%
