...
files <- list.files('res', full.names = T, recursive = T) %>%
  subset(., grepl('txt', .) & grepl('extended\\/FIGUREA4', .))
...
df <- temp[(index_vacc):(index[2]-2), ] %>%
  ...
  mutate(DEP = case_when(DEP == 'diffaod550_pop' ~ 'AOD (change)',
                         DEP == 'diffcase_pop' ~ ' new Covid19 cases',
                         DEP == 'diffmobility' ~ 'Mobility (change)',
                         DEP == 'diffntl_pop' ~ 'NTL (change)'))
